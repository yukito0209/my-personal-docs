...
const QQChatPanel: React.FC = () => {
  ...
  const handleMouseDownOnHeader = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (chatPanelRef.current) {
      dragStartOffsetRef.current = {
        x: e.clientX - chatPanelRef.current.offsetLeft,
        y: e.clientY - chatPanelRef.current.offsetTop,
      };
    }
  }, []);
  ...
  useEffect(() => {
    const handleMouseMoveDrag = (e: MouseEvent) => {
      ...
      newTop = Math.max(10, Math.min(newTop, windowDimensions.height - chatPanelRef.current.offsetHeight - 10));
      newLeft = Math.max(10, Math.min(newLeft, windowDimensions.width - chatPanelRef.current.offsetWidth - 10));
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="qq-panel-title">AI助手聊天</span>
        </div>
        <button 
          onClick={closeChat} 
      ...
      <div className="qq-panel-content">
        {/* 左侧助手列表 */}
        <div className="qq-assistant-list" style={{ width: ASSISTANT_LIST_WIDTH }}>
